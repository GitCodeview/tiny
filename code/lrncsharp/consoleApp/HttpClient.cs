...
namespace consoleApp
{
    public class WebClientHelper
    {
        ...
        public static string DownloadStringWithBid(string url, string bid)
        {
            WebClient wc = new WebClient();
            wc.Headers.Add("Bid", bid);
            //wc.BaseAddress = url;   //设置根目录
            wc.Encoding = Encoding.UTF8;    //设置按照何种编码访问，如果不加此行，获取到的字符串中文将是乱码
            string str = wc.DownloadString(url);
            return str;
        ...
        public static void DownloadStringAsync(string url)
        {
            WebClient wc = new WebClient();
            wc.Encoding = Encoding.UTF8;    //设置按照何种编码访问，如果不加此行，获取到的字符串中文将是乱码
            wc.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36");
            wc.DownloadStringCompleted += DownloadStringCompletedHandler;
            wc.DownloadStringAsync(new Uri(url));
            Console.WriteLine("下载中。。。");
        }

        private static void DownloadStringCompletedHandler(object sender, DownloadStringCompletedEventArgs e)
        {
    ...
    class HttpClient
    {
        public static string DownloadString(string url)
        {
            return WebClientHelper.DownloadString(url);
        }

        public static void DownloadStringAsync(string url)
        {
            WebClientHelper.DownloadStringAsync(url);
        }

        public static string DownloadStringWithBid(string url, string bid)
        {
            return WebClientHelper.DownloadStringWithBid(url, bid);
        }

        public static void DownloadFile(string url, string filename)
        {
            WebClientHelper.DownloadFile(url, filename);
            //WebClientHelper.DownloadFileAsync(url, filename);
        }
